...
const LibraryItem = ({item, onDelete, editStateFunc}) =>
{
    ...
    const activateEditMode = () => {setEditStateBool(state => !state)};


    const options = {mode: 'Edit', isbn: isbn, className: 'EditForm'};
    const editForm = <AddItemForm stateModifier={editStateFunc} 
    options={options} toggleForm={activateEditMode}></AddItemForm>;
...
<button onClick={() => onDelete(isbn)} style={{ padding: '5px 10px' }}>Delete</button>
